...
export class CarUpdateComponent implements OnInit {
  ...
  update() {
    if (!this.carUpdateForm.valid) {
      this.toastrService.error("Form is Incorrect", "Invalid Form")
    } else {
      ...
      if (!carInfoChanged && !carImagesChanged) {
        this.toastrService.error("You haven't made any changes", "Not Updated")
      } else {
        if ((this.carImages.length + this.uploadFiles.length) > 5) { //Max 5 Image
          this.toastrService.error("You can upload upto 5 Images", "No Vehicle added");
        } else {
          this.updateCarImages(deletedImages, this.uploadFiles).then((updateCarImageResult) => {
            this.carService.update(carUpdateModel).subscribe(updateSuccess => {
              if (updateCarImageResult.length > 0) {
                this.toastrService.warning("The Car was Updated Successfully,but Some images could not be Updated " + updateCarImageResult, "Operation Partially Updated")
                this.closeCarUpdateModal();
              } else {
                this.toastrService.success("Car Updated Succefully", "Car Updated")
                this.closeCarUpdateModal();
              }
            }, errorResponse => {
              this.errorService.showBackendError(errorResponse, "Failed to Updated");
            })
          });
        }
      }
      ...
      deletePromise.then(() => {
        ...
        uploadPromise.then(() => {
          ...
          if (unDeletedImagesList.length > 0) {
            resultString += "Undeletable Image Id: " + unDeletedImagesList
          }
          if (unUploadedFileList.length > 0) {
            resultString += "Image not Uploaded: " + unUploadedFileList
          }
      ...
      if ((this.carImages.length + this.uploadFiles.length) < 5) {
        for (let i = 0; i < imageList.length; i++) {
          ...
          if (preselectedFile === undefined) {
            ...
            this.toastrService.warning("You have already added this image to the list", "Already Listed");
          }
        }
      } else {
        this.toastrService.error("You can add upto 5 images", "Failed to add Images");
      }
    }
  }
    ...
    return new Promise<boolean>((result) => {
      this.checkFileMimeType(image).then((successStatus) => {
        if (successStatus) {
          ...
          this.toastrService.error("You can only add Image File", "File not Attached");
          result(false);
        }
      })
    })
