...
export class CarAddComponent implements OnInit {
  ...
  add() {
    if (!this.carAddForm.valid) {
      this.toastrService.error("Your form is Invalid", "Invalid form");
    } else {
      ...
      this.carService.add(carModel).subscribe(carAddSuccessResponse => {
        if (this.carImagesFiles.length === 0) {  //No pictures, just car added
          this.toastrService.success("New car added sucessfully", "Opration Successfull");
          this.closeCarAddModal();
        }
        else {  //If car pictures will be uploaded
          if (this.carImagesFiles.length > 5) { //Max 5 Image
            this.toastrService.error("You can Upload upto 5 Images", "Cars not Added");
          } else {
            this.uploadAllImagesToServer(this.carImagesFiles, carAddSuccessResponse.data).then((unUploadFileList) => {
              ...
              if (unUploadedFiles.length === 0) {
                this.toastrService.success("New Car and Images Added Successfully", "Opration Successfull");
                this.closeCarAddModal();
              } else {
                ...
                this.toastrService.warning("The New Car has been Successfully Added, But Some images could not be loaded: " + failFileNameMessage, "Operation Partially Successful");
                this.closeCarAddModal();
              }
            })
        ...
        this.errorService.showBackendError(errorResponse, "Failed to Add Car");
      })
    }
  }
      ...
      if (this.carImagesFiles.length < 5) {
        for (let i = 0; i < imageList.length; i++) {
          ...
          if (preselectedFile === undefined) {
            ...
            this.toastrService.warning("You have already added this image to the list", "Alredy Listed");
          }
        }
      } else {
    ...
    return new Promise<boolean>((result) => {
      this.checkFileMimeType(image).then((successStatus) => {
        if (successStatus) {
          ...
          this.toastrService.error("You can only upload Image File", "File Not Attached");
          result(false);
        }
      })
    })
