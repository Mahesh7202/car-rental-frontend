...
export class CarDetailsComponent implements OnInit {
  ...
  isReviewsPresent: boolean = false;
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe((params) => {
      if (params['carid']) {
        ...
        this.getReviews(params['carid']);
      }
    });
  }
  getReviews(carid: number) {
    this.reviewService.getReviewsByCarId(carid).subscribe((res) => {
      this.reviews = res.data;
      this.isReviewsPresent = true;
    });
  }
  ...
  calculateRatingDistribution(): number[] {
    const distribution: number[] = [0, 0, 0, 0, 0]; // Initialize with 5 zeros for each star rating
